...
do Google Earth, medimos a distância de cada ponto à antena colocando essas métricas num 
...
Ambos os sistemas permitem analisar a propagação dos sinais codificados com esquemas de modulação QPSK, 64-QAM e 256-QAM e para canais AWGN (Ruído Gaussiano Branco Aditivo) e canais com fading de Rayleigh, este que representa o pior caso para comunicação.
Além disto foi simulada a diversidade espacial na recepção, utilizando 4 antenas.
Nas Figuras \ref{fig:BlocosOFDM}  e \ref{fig:BlocosSCFDE} é apresentado os diagramas de blocos do simulador tanto para OFDM quanto para SC-FDE onde é possível ver as diferenças entre ambos.
...
O sistema de transmissão implementado tem uma estrutura de blocos dividida em Emissor, Canal e Recetor. Estes que são comuns em ambos os modos (OFDM e SC-FDE). O emissor gera os dados e a
...
\begin{itemize}
    \item Ruído: AWGN (Additive White Gaussian Noise), este que demonstra o melhor cenário possível, contendo apenas ruído térmico e a densidade espectral de potência é constante;
...
Como foi abordado na componente teórica da unidade curricular a principal distinção entre estas duas técnicas está no processamento do sinal 
...
com isso podemos calcular o bit error rate (BER), tendo uma meta de $10^{-4}$, analisando qual o valor de SNR necessário para obter este desempenho.
