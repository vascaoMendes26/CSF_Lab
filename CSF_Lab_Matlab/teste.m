...
L = 4;              % Diversidade na receção
FASE = 1;           % 1 = Ideal, 2 = Com Amplificador (Rapp)
...
for nn=1:NSlot
    ...
    elseif (FASE == 2)
        ...
        elseif strcmp(SYSTEM, 'SCFD')
            an_Tx_Original = Ak_Tx; % Em SC-FDE os símbolos já estão no tempo
        end
        ...
        % Fórmula Rapp usando o fixed_sat_level calculado acima
    ...
    % --- CANAL E RECETOR ---
     for nEN=1:NEN
        ...
        % ... (dentro do loop do recetor) ...

        % Equalização MRC (Maximal Ratio Combining)
        YIk=0;
        for l=1:L
...
for l=0:L-1
    Pb_tr = Pb_tr + nchoosek(L-1+l,l)*((1+aux)/2).^l;
end
Pb_tr = Pb_tr .* ((1-aux)/2).^L;

% AWGN
PbAWGN = qfunc(sqrt(2*L*en));
...
% --- PLOT ---
figure;
semilogy(EN, Pb, 'g-*', 'LineWidth', 1.5); hold on;
semilogy(EN, PbAWGN, 'b:', 'LineWidth', 1.5);
semilogy(EN, Pb_tr, 'r*:', 'LineWidth', 1.5);
legend('Simulado', 'Teórico AWGN', 'Teórico Rayleigh');
title(['BER vs SNR | ' SYSTEM ' | ' mode ' | p=' num2str(p_rapp) ' | IBO=' num2str(IBO_dB) 'dB']);
